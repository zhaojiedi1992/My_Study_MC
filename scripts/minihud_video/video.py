...
def build_transition_filter(
    ...
    for index in range(1, len(durations)):
        ...
        parts.append(
            f"{audio_in}[{index}:a]"
            f"acrossfade=d={transition:.3f}:c1=tri:c2=tri{audio_out}"
        )
        video_in = video_out
        audio_in = audio_out
...
def _measured_loudnorm_filter(measurements: dict[str, str]) -> str:
...
def compose_master(build_dir: Path) -> Path:
    ...
    measurements = _measure_loudness(master)
    ...
    subprocess.run(
        [
            ...
            "copy",
            "-af",
            _measured_loudnorm_filter(measurements),
            "-c:a",
            "aac",
            "-b:a",
            "192k",
...
def burn_subtitles(build_dir: Path, clean: Path) -> Path:
    ...
    style = (
        "FontName=Noto Sans CJK SC,FontSize=42,"
        "PrimaryColour=&H00FFFFFF,OutlineColour=&H0010182A,"
        "BorderStyle=1,Outline=2,Shadow=0,Alignment=2,MarginV=72"
    )
    subprocess.run(
        [
            FFMPEG,
...
def create_contact_sheet(build_dir: Path) -> Path:
    ...
    subprocess.run(
        [
            ...
            "-y",
            "-i",
            str(build_dir / "minihud-bilibili.mp4"),
            "-vf",
