...
==================================================
- 快速——编写时考虑了性能和可扩展性。
- 可靠——受到数千名服务器管理员和最大的服务器网络的信任。
- 易于使用- 使用命令、直接在配置文件中或使用 Web 编辑器设置权限。
- 灵活——支持多种数据存储选项，并适用于多种不同类型的服务器。
- 广泛- 大量自定义选项和设置，可以根据您的服务器进行更改。
- 免费——可以免费下载和使用，并且经过许可，因此可以永远免费。
...
.. code-block:: bash 
    ...
    # 其他的分区安装
    cp -r  /home/mc/instances/dl1/plugins/LuckPerms* /home/mc/instances/dp1/plugins/
    cp -r  /home/mc/instances/dl1/plugins/LuckPerms* /home/mc/instances/sc1/plugins/
    cp -r  /home/mc/instances/dl1/plugins/LuckPerms* /home/mc/instances/sc2/plugins/
    sed -i 's@server: dl1@server: dp1@g'  /home/mc/instances/dp1/plugins/LuckPerms/config.yml
    sed -i 's@server: dl1@server: sc1@g'  /home/mc/instances/sc1/plugins/LuckPerms/config.yml
    sed -i 's@server: dl1@server: sc2@g'  /home/mc/instances/sc2/plugins/LuckPerms/config.yml
...
.. code-block:: bash 
    ...
    # sleep 1 是为了等待，避免权限插入失败。
    cat command.list.txt |while read file; do bash exec_one_cmd.sh  "$file" ; sleep 1 ; done
...
.. code-block:: bash 
    ...
    # 方式3 ，查看以特定player执行一个命令，需要的权限信息，非常推荐的。
